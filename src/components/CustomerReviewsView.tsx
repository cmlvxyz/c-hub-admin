...
interface CustomerReviewsViewProps {
  ...
  onModerateReview: (reviewId: string, status: 'published' | 'hidden' | 'rejected') => void;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 pt-1">
                    <button
                      onClick={() => onModerateReview(review.id, 'published')}
                      disabled={review.status === 'published'}
                      className={`px-2.5 py-1 rounded-md text-[10px] font-semibold flex items-center gap-1 transition-colors ${
                        review.status === 'published'
                          ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 cursor-default'
                          : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-emerald-100 dark:hover:bg-emerald-950 disabled:opacity-50'
                      }`}
                    >
                      <CheckCircle2 className="w-3 h-3" /> Approve
                    </button>
                    <button
                      onClick={() => onModerateReview(review.id, 'hidden')}
                      disabled={review.status === 'hidden'}
                      className={`px-2.5 py-1 rounded-md text-[10px] font-semibold flex items-center gap-1 transition-colors ${
                        review.status === 'hidden'
                          ? 'text-slate-600 bg-slate-100 dark:bg-slate-800 cursor-default'
                    ...
                    <button
                      onClick={() => onModerateReview(review.id, 'rejected')}
                      disabled={review.status === 'rejected'}
                      className={`px-2.5 py-1 rounded-md text-[10px] font-semibold flex items-center gap-1 transition-colors ${
                        review.status === 'rejected'
                          ? 'text-red-600 bg-red-50 dark:bg-red-950/40 cursor-default'
                          : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-red-100 dark:hover:bg-red-950 disabled:opacity-50'
                      }`}
                    >
                      <Eye className="w-3 h-3" /> Reject
                    </button>
                    <button
                      onClick={() => onDeleteReview(review.id)}
                      className="ml-auto px-2.5 py-1 rounded-md text-[10px] font-semibold flex items-center gap-1 text-slate-500 bg-transparent hover:bg-red-50 dark:hover:bg-red-950/40 hover:text-red-600 transition-colors"
                    >
